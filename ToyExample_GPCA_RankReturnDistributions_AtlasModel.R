...
nd<-9 # nth Dyadic partition for Omega grid
...
h<-1e-7 # central difference step size
...
# Plot Convex PCs
plot(omega_grid[2:length(pvals)],gpc1,type="l", col="red", 
     ylim=c(min(gpcs),max(gpcs)), ylab = "Convex PCs", xlab = "Omega")
lines(omega_grid[2:length(pvals)],gpc2, type="l", col="blue")
...
for(epsilon in epsilons){
  for(i in seq(0,1,1)){
    if(i==0){
      x_m<-x0-epsilon*gpcs[,pertdir]
    }else{
      x_p<-x0+epsilon*gpcs[,pertdir]
    }
  }
