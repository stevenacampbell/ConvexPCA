...
sourceCpp("SupportingCode/HelperFunctions_CPCA.cpp")
...
eval_CPCA_obj_closure <- function(theta,B,x0,A,b,X) {
  function(theta) eval_CPCA_obj_sph(theta,B,x0,A,b,X)
}

eval_grad_CPCA_obj_parallel_closure <- function(theta,B,h,x0,A,b,X,nthreads) {
  function(theta) eval_grad_GPCA_obj_sph_parallel(theta,B,h,x0,A,b,X,nthreads)
}
...
CPCA<-function(npcs,X,x0,A,b,h,nthreads){
  ...
  for(i in seq(1,npcs,1)){
    ...
    eval_grad_f <- eval_grad_CPCA_obj_parallel_closure(theta,B,h,x0,A,b,X,nthreads)
